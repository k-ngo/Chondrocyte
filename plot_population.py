...
# Load datasets
detected_files = glob.glob(os.path.join(input_folder, '*.npy'))
all_currents = np.load(detected_files[0])
all_voltage_and_ion = np.load(detected_files[1])
all_voltage_and_ion_ss = np.load(detected_files[2])
all_parameters = np.load(detected_files[3])
